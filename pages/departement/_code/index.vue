  ...
  <div>
    ...
    <div class="mt-16 font-body grid grid-cols-8 grid-rows-6 gap-8">
      ...
      <div class="bg-black text-white p-4 col-span-2 row-span-2">
        ...
        <div class="text-4xl font-semibold">{{departement.communes_aggregate.aggregate.sum.population | numeralFormat}}</div>
      </div>
      ...
      <!-- <div class="bg-gray-800 text-white p-4 col-span-4 row-span-4">
        ...
        <ul>
          <li v-for="(c, i) in departement.communes" :key="i" class="text-xl leading-none my-2">
            <n-link :to="`/communes/${c.insee_com}/`" class="collectivite-link">{{c.nom}}</n-link>
          </li>
        </ul>
      </div> -->

      <div class="p-4 col-span-4 bg-gray-600 text-lg font-semibold text-white space-x-4">
        <a :href="`https://fr.wikipedia.org/wiki/${departement.nom}`" target="_blank" class="t">Wikipedia
        ...
        <a :href="`https://www.google.com/maps/place/${departement.nom}`" target="_blank" class="t">Google Maps
